...
object d08:
  ...
  def run(): Unit =
    val input: Array[Array[String]] = 
      util.readLines("input/8").map(_.split(" "))
    val screenInput = input.map(xs => xs.take(xs.indexOf("|")))
    val screenOutput = input.map(xs => xs.drop(xs.indexOf("|") + 1))
    part1(screenOutput)
    part2(screenInput, screenOutput)

  def part1(input: Array[Array[String]]): Unit =
  ...
  def part2(input: Array[Array[String]], output: Array[Array[String]]): Unit =
